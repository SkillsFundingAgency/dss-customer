...
{
    class SearchCustomerHttpTriggerService
    {
        public List<Models.Customer> SearchCustomer(HttpRequestMessage httpReq)
        {
            var queryStrings = httpReq.GetQueryNameValuePairs();
            var _givenname = queryStrings.FirstOrDefault(kv => string.Compare(kv.Key, "GivenName", true) == 0);
            var _familyname = queryStrings.FirstOrDefault(kv => string.Compare(kv.Key, "FamilyName", true) == 0);
            var result = CreateTempCustomers().Where(x => x.GivenName == _givenname.Value || x.FamilyName == _familyname.Value).ToList();
            return result;
        }
